...
#Region "Properties"
    ...
    <BrowsableAttribute(True), EditorAttribute(GetType(FileFolderEditor), GetType(System.Drawing.Design.UITypeEditor))>
    ...
    Private m_Prefix As String = ""  '* initialize because it is used as start string in StoreValue
    ...
    Public Property CreateNewLogFileAtMaxPoints As Boolean
        ...
        Set(value As Boolean)
            ...
            If Not m_CreateNewLogFileAtMaxPoints Then
                m_LogFileCount = 0
            End If
        End Set
    End Property
            ...
            If m_EnableLogging Then
                ...
                SyncLock (SwLock)
                    ...
                    If m_FileNameIncludesDate Then
                        '* v3.99y beta 14
                        If m_CreateNewLogFileDaily AndAlso (String.Compare(PreviousDate, DateTime.Now.ToString("dd-MMM-yyyy"), True) <> 0) Then
                            CreateNewLog()
                        End If
                        Dim Extension As String
                        Dim PeriosPos As Integer = fileNameToUse.LastIndexOf(".")
                        If PeriosPos >= 0 Then
