...
#Region "Basic Properties"
        ...
        Set(ByVal value As String)
            If value <> m_Value Then
                If Not String.IsNullOrEmpty(value) Then
                    m_Value = value
                    '* V3.99y
                    Double.TryParse(value, Globalization.NumberStyles.Number Or Globalization.NumberStyles.AllowParentheses,
                                    System.Globalization.NumberFormatInfo.CurrentInfo, ValueAsDouble)
                    UpdateText()
                    OnValueChanged(EventArgs.Empty)
                Else
                    '* version 3.99f
                    If Not String.IsNullOrEmpty(m_Value) Then OnValueChanged(EventArgs.Empty)
                    m_Value = ""
                    MyBase.Text = ""
                End If
    ...
    Private m_VisibleControl As VisibleControlOptions = VisibleControlOptions.Always
    Public Property VisibleControl As VisibleControlOptions
        Get
            Return m_VisibleControl
        End Get
        Set(value As VisibleControlOptions)
            If m_VisibleControl <> value Then
                m_VisibleControl = value
                If m_VisibleControl = VisibleControlOptions.Always Then
                    Me.Visible = True
                End If
            End If
        End Set
...
#Region "Private Methods"
    Private Sub UpdateText()
        ...
        If m_ShowValue Then
            ...
            If Not String.IsNullOrEmpty(m_NumericFormat) Then
                If Double.TryParse(Value, Globalization.NumberStyles.Number Or Globalization.NumberStyles.AllowParentheses,
                                   System.Globalization.NumberFormatInfo.CurrentInfo, v) Then
                    Try
    ...
    Private Sub UpdateVisible()
        If m_VisibleControl <> VisibleControlOptions.Always Then
            If m_VisibleControl = VisibleControlOptions.AboveLimits Then
                Me.Visible = (ValueAsDouble > ValueLimitUpper)
            ElseIf m_VisibleControl = VisibleControlOptions.BelowLimit Then
                Me.Visible = (ValueAsDouble < ValueLimitLower)
            ElseIf m_VisibleControl = VisibleControlOptions.WithinLimits Then
...
#Region "Subscribing and PLC data receiving"
    ...
    '* V3.99y Beta 19 - Added to make it easier for writing through the SubscriptionHandler
    Private Sub CreateSubscriptionHandler()
...
#Region "Keypad popup for data entry"
    Private KeypadPopUp As MfgControl.AdvancedHMI.Controls.KeypadV2
