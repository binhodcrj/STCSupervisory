...
'*
'* This class creates and interface between a driver and the AdvancedHMI
'* visual controls.
'*
...
#Region "Tag Alias"
    Private TagAliasFileRead As Boolean
    Private TagAlias As Dictionary(Of String, String)
    ...
    Public Overrides Function BeginWrite(startAddress As String, numberOfElements As Integer, dataToWrite() As String) As Integer
        Return MyBase.BeginWrite(ReadTagAliases(startAddress), numberOfElements, dataToWrite)
    End Function

    Public Overrides Function Subscribe(ByVal plcAddress As String, ByVal numberOfElements As Int16, ByVal pollRate As Integer, ByVal callback As EventHandler(Of MfgControl.AdvancedHMI.Drivers.Common.PlcComEventArgs)) As Integer
        Return MyBase.Subscribe(ReadTagAliases(plcAddress), numberOfElements, pollRate, callback)
    End Function


    Private m_TagAliasFile As String = ".\TagAlias.txt"
    Public Property TagAliasFile As String
        Get
            Return m_TagAliasFile
        End Get
        Set(value As String)

            m_TagAliasFile = value
        End Set
    End Property
    '********************************************************
    ...
    '********************************************************
    Private Function ReadTagAliases(ByVal address As String) As String
        If Not TagAliasFileRead Then
            If Not String.IsNullOrEmpty(m_TagAliasFile) AndAlso (System.IO.File.Exists(m_TagAliasFile)) Then
                TagAlias = New Dictionary(Of String, String)(StringComparer.InvariantCultureIgnoreCase)
                Using sr As New System.IO.StreamReader(m_TagAliasFile)
                    Dim Line As String
                    Dim Items() As String
        ...
        If TagAlias IsNot Nothing Then
            If TagAlias.ContainsKey(address) Then
                Return TagAlias(address)
            End If
        End If
