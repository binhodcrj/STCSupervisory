    ...
    Private Sub OpenForm()
        If FormToOpen IsNot Nothing Then
            ...
            If index < p.Length Then
                ...
                '*V3.99y - moved before above loop in case button is on embedded container such as a panel
                f1 = DirectCast(p(index).GetValue(My.Forms, Nothing), Form)
                f1.Show()
                f1.BringToFront()

                Dim mp = pf.GetType.GetProperty("MenuPosition")
                Dim MenuPosition As MfgControl.AdvancedHMI.Controls.MenuPos = MfgControl.AdvancedHMI.Controls.MenuPos.LeftOrTop
                If mp IsNot Nothing Then
                    MenuPosition = mp.GetValue(pf, Nothing)
                End If
                '* V3.99y Beta 32
